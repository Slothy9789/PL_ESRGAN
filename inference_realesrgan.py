    ...
    for idx, path in enumerate(paths):
        ...
        if path.endswith(".tif"):
            ...
            for i in range(1, num):  # 0 is background
                ...
                if area < min_area:  # 如果黑色区域小于阈值
                    continue
                if h < min_h:       # 如果黑色区域的高度小于阈值
                    continue
                if w < min_w:       # 如果黑色区域的宽度小于阈值
                    continue
                aspect = h / (w + 1e-6)   # 如果黑色区域的高宽比小于阈值
                if aspect < min_aspect:
                    continue
                # must roughly touch both top and bottom (a seam-like stripe)
                touches_top    = (y <= border_tol)   # 如果黑色区域触顶
                touches_bottom = (y + h >= H - border_tol)  # 如果黑色区域触底
            ...
            try:
                ...
                if args.geo_info == True:
                    ...
                    band1.WriteArray(output, 0, 0)  # 原图是整数，乘个0.1，与浮点数可以相加
                    New_YG_dataset = None
